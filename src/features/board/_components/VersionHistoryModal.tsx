...
export function VersionHistoryModal({ boardId, isOwner }: VersionHistoryModalProps) {
  ...
  const filteredVersions = useMemo(() => {
    ...
    return versions.filter((v) =>`version${v.version}`.includes(q) ||v.createdBy?.name?.toLowerCase().includes(q) || // filter the versions based on the version number or the name of the user or the date of the version
      new Date(v.createdAt).toLocaleString().toLowerCase().includes(q)); // filter the versions based on the date of the version
  ...
  return (
    <>
      ...
      <AlertDialog open={!!restoringVersion} onOpenChange={(open) => !open && setRestoringVersion(null)}>
        <AlertDialogContent className="max-w-md rounded-2xl z-[999999]">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-xs text-muted-foreground leading-relaxed">
              This will overwrite the current canvas with the snapshot saved on{" "}
              {restoringVersion && new Date(restoringVersion.createdAt).toLocaleString()}. Any unsaved changes on the current canvas will be replaced.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="mt-2">
            <AlertDialogCancel disabled={isRestoring} className="rounded-xl h-9 text-xs">
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              ...
              className="rounded-xl h-9 text-xs bg-blue-600 hover:bg-blue-500 text-white font-medium gap-1.5"
            >
